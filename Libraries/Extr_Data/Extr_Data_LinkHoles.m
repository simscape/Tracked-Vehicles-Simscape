...
theta = (-89:5:+90)' * pi/180; % CCW
rght_end = repmat([hl 0], length(theta*2), 1) + hw * [cos(theta), sin(theta)];
left_end = flipud(rght_end) * diag([-1 1]);
boundary = [rght_end; left_end];

% Circular holes
%theta = (270:-1:-90)' * pi/180; % CW
theta = (270:-10:-90)' * pi/180; % CW
hole = [[0 -hw]; r * [cos(theta), sin(theta)]; [0 -hw]];
holes = kron(linspace(-hl, +hl, num_holes)', repmat([1 0], length(theta)+2, 1)) ...
