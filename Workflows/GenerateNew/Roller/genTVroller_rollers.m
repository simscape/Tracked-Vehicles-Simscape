function [newUndercFile, newUndercSubs] = genTVroller_rollers(nRollL,nRollU,nIdler,newFrameFile,newFrameSub)

% Source files: Copy to new location and modify
refUndercFile = 'sm_rollerV_lib_underc_i1u2l7';
refUndercSubs = 'Undercarriage Roller Contact Point Cloud';
...
% DUE TO MASK BUG - need to use "Inherit"
bh_ssbu = Simulink.findBlocks([newUndercFile '/' newRollerSubs],'BlockType','SimscapeBus','ConnectionType','.*Roller.*',f); 
%bh_ssbu = Simulink.findBlocks([newUndercFile '/' newRollerSubs],'BlockType','SimscapeBus','ConnectionType','Inherit: auto',f);
bh_acLR = Simulink.findBlocks([newUndercFile '/' newRollerSubs],'Name','Angle Constraint Lower[0-9]',f);
...
for i = 1:length(bh_rvLR)
    ...
    % Delete Angle Constraint Lower
    if(i>1)
        line_h = get_param(bh_acLR(i),'LineHandles');
        delete_line(line_h.LConn)
        delete_line(line_h.RConn)
        delete_block(bh_acLR(i));
    end
end
...
set_param(bh_ssbu,'ConnectionType',['Bus: CBO_Roller' num2str(nRollL+2)])
...
% Find Angle Constraint for copy/paste
bh_acLR = Simulink.findBlocks([newUndercFile '/' newRollerSubs],'Name','Angle Constraint Lower[0-9]*',f);

% Find Connection Labels for Sprocket and Idler for copy/paste
bhcnSpr = Simulink.findBlocks([newUndercFile '/' newRollerSubs],'BlockType','ConnectionLabel','Label','Spr',f);
bhcnIdl = Simulink.findBlocks([newUndercFile '/' newRollerSubs],'BlockType','ConnectionLabel','Label','Idl',f);
...
pos_acLR = get_param(bh_acLR,'Position');
...
for i = 2:nRollL    
    ...
    bh_soLR_all(i) = bh_soLRi;
    bNameLR = sprintf('Roller\nLower %s',pad(num2str(i),nCharNameLR,'left','0'));
    pos_soLRi = pos_soLR+[i-1 0 i-1 0]*rvl_offset_x;
    set_param(bh_soLRi,'Name',bNameLR,'Position',pos_soLRi)

    % Copy/Paste Angle Constraint, set position and name
    bh_acLRi = add_block(getfullname(bh_acLR),getfullname(bh_acLR),'MakeNameUnique','on');
    bNameLR = sprintf('Angle\nConstraint\nLower%s',pad(num2str(i),nCharNameLR,'left','0'));
    pos_acLRi = pos_acLR+[i-1 0 i-1 0]*rvl_offset_x;
    set_param(bh_acLRi,'Name',bNameLR,'Position',pos_acLRi)
    ...
    sl_px1 = ((pos_rvLRi(1)+pos_rvLRi(3))-sl_w)/2+9;
    sl_px2 = sl_px1+sl_w;
    sl_py1 = pos_rvLRi(2)+sl_offset_y;
    sl_py2 = sl_py1+sl_h;
    posNewSL = [sl_px1 sl_py1 sl_px2 sl_py2];
    sl_name = sprintf('PS-Simulink Converter LR%i',j);
    ...
    ph_acLRi = get_param(bh_acLRi,'PortHandles');
    add_line([newUndercFile '/' newRollerSubs],ph_rvLRi.RConn(1), ph_acLRi.RConn(1),'autorouting','on')
    ph_soLRlprev = get_param(bh_soLR_all(i-1),'PortHandles');
    add_line([newUndercFile '/' newRollerSubs],ph_soLRlprev.LConn(1), ph_acLRi.LConn(1),'autorouting','on')
...
% Reconnect Sprocket and Idler Connection Labels to Simscape Bus
for i = 1:length(bhcnSpr)
    line_h = get_param(bhcnSpr(i),'LineHandles');
    % Connection label we need to connect will have a line
    if(line_h.LConn == -1)
        ph_cnSpr = get_param(bhcnSpr(i),'PortHandles');
        add_line([newUndercFile '/' newRollerSubs],ph_cnSpr.LConn(1),ph_ssbu.LConn(j+1))
        set_param(bhcnSpr(i),'Position',...
            posNewCN+[0 25 0 25]*1);
    end
end

for i = 1:length(bhcnIdl)
    line_h = get_param(bhcnIdl(i),'LineHandles');
    % Connection label we need to connect will have a line
    if(line_h.LConn == -1)
        ph_cnIdl = get_param(bhcnIdl(i),'PortHandles');
        add_line([newUndercFile '/' newRollerSubs],ph_cnIdl.LConn(1),ph_ssbu.LConn(j+2))
        set_param(bhcnIdl(i),'Position',...
            posNewCN+[0 25 0 25]*2);
    end
end

% Resize Simscape Bus block based on number of ports
pos_ssbu   = get_param(bh_ssbu,'Position');
%ssbu_py(i) = pos_ssbu(2); 
pos_ssbu_new    = pos_ssbu;
pos_ssbu_new(4) = pos_ssbu(2)+25*(nRollL+2);
...
% Eliminate other subsystems
bh_subs = Simulink.findBlocks(newUndercFile,'BlockType','SubSystem',f);
bh_news = Simulink.findBlocks(newUndercFile,'Name',newUndercSubs,f);
bh_2del = setdiff(bh_subs,bh_news);
if(~isempty(bh_2del))
    for bh_i = 1:length(bh_2del)
        delete_block(bh_2del(bh_i))
    end
end
